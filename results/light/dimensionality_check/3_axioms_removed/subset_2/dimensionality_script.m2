...
remainingAxioms = delete(S * r^2 - qc^2 * ap^2 * sintheta^2, remainingAxioms);
remainingAxioms = delete(100*dA - 2*(314)*(r^2)*sintheta*dtheta, remainingAxioms);
remainingAxioms = delete(4 - 3*sintheta^3 * dtheta, remainingAxioms);
...
-- Output
f = openOut "results/light/dimensionality_check/3_axioms_removed/subset_2/dimensionality_output.txt";
...
f << "Removed axioms: " << toString({S * r^2 - qc^2 * ap^2 * sintheta^2, 100*dA - 2*(314)*(r^2)*sintheta*dtheta, 4 - 3*sintheta^3 * dtheta}) << endl;
