...
measuredVariables = toList([v, c, m, m0, u0]);
nonMeasuredVariables = toList([dt, F0, F, dt0, L0, L, u]);
...
f = openOut "results/relativistic_laws_updated/projection/target_2/projection_output.txt";
...
qList = toList([c^2*m0^2*u0-c^2*u0*m^2+v^2*u0*m^2]);
