...
remainingAxioms = delete(F0*dt0 - 1, remainingAxioms);
...
-- Output
f = openOut "results/relativistic_laws_updated/dimensionality_check/3_axioms_removed/subset_11/dimensionality_output.txt";
...
f << "Removed axioms: " << toString({F0*dt0 - 1, c^2*dt^2 - 4*L0^2 - v^2*dt^2, u0*dt0 - u*dt}) << endl;
