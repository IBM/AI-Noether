...
remainingAxioms = delete(c*dt0 - 2*L0, remainingAxioms);
remainingAxioms = delete(c^2*dt^2 - 4*L0^2 - v^2*dt^2, remainingAxioms);
remainingAxioms = delete(u0*dt0 - u*dt, remainingAxioms);
...
-- Output
f = openOut "results/relativistic_laws_updated/dimensionality_check/3_axioms_removed/subset_27/dimensionality_output.txt";
...
f << "Removed axioms: " << toString({c*dt0 - 2*L0, c^2*dt^2 - 4*L0^2 - v^2*dt^2, u0*dt0 - u*dt}) << endl;
