...
remainingAxioms = delete(sigma - e * ph * mup - e * n * muN, remainingAxioms);
remainingAxioms = delete(H * (ph + beta * n)^2 * e - r * ph + r * beta^2 * n, remainingAxioms);
...
-- Output
f = openOut "results/photoHall/dimensionality_check/2_axioms_removed/subset_21/dimensionality_output.txt";
...
f << "Removed axioms: " << toString({sigma - e * ph * mup - e * n * muN, H * (ph + beta * n)^2 * e - r * ph + r * beta^2 * n}) << endl;
