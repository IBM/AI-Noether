...
remainingAxioms = delete(pm^2*(c^2 - vm^2) - mm^2*vm^2*c^2, remainingAxioms);
remainingAxioms = delete(2*Em*Er - Ec^2 + Em^2 + Er^2, remainingAxioms);
remainingAxioms = delete(pc^2*(c^2 - vc^2) - mc^2*vc^2*c^2, remainingAxioms);
...
-- Output
f = openOut "results/inelastic_collision/dimensionality_check/3_axioms_removed/subset_22/dimensionality_output.txt";
...
f << "Removed axioms: " << toString({pm^2*(c^2 - vm^2) - mm^2*vm^2*c^2, 2*Em*Er - Ec^2 + Em^2 + Er^2, pc^2*(c^2 - vc^2) - mc^2*vc^2*c^2}) << endl;
