...
R = CC[Eki, Ekf, Ugi, Ugf, G, M, m, ve, r, MonomialOrder => Lex];

remainingAxioms = toList([Eki - 1/2 * m * ve^2, Ekf - 0, Ugi * r + G * M * m, Eki + Ugi - Ekf - Ugf]);
qList = toList([(20000178/10000000)*G*M*m - (10008172/10000000)*m*ve^2*r]);
...
f = openOut "results/escape_velocity/abduction/noisy/noise_1e-8/1_axiom(s)_removed/combo_4/decomposition/witness_sets/witness_set.txt";
f << "variable ordering: ";
...
close f;
print("Witness set computation complete. Output written to results/escape_velocity/abduction/noisy/noise_1e-8/1_axiom(s)_removed/combo_4/decomposition/witness_sets/witness_set.txt");
