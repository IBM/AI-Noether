...
remainingAxioms = delete(m1*d1-m2*d2, remainingAxioms);
remainingAxioms = delete(Fc - m2*d2*w^2, remainingAxioms);
remainingAxioms = delete(Fc - Fg, remainingAxioms);
...
-- Output
f = openOut "results/real/kepler/dimensionality_check/3_axioms_removed/subset_4/dimensionality_output.txt";
...
f << "Removed axioms: " << toString({m1*d1-m2*d2, Fc - m2*d2*w^2, Fc - Fg}) << endl;
